...
A thin wire is coiled around a cylindrical tube of radius $R$. An \emph{ideal solenoid}\index{ideal solenoid} is infinitely long and tightly wound, having cylindrical geometry and purely azimuthal current.
...
So each (Cartesian) component of $\mathbf{E}$ and $\mathbf{B}$ satisfies the \emph{wave equation}\index{wave equation}
\[
...
\]
This is because light is an \emph{electromagnetic wave}\index{electromagnetic wave} involving oscillations of $\mathbf{E}$ and $\mathbf{B}$. Depending on the wavelength, EM waves can be radio waves, microwaves, infrared, ultraviolet, X-rays and gamma rays.

\subsubsection{Plane Electromagnetic Waves}
\label{subsub:plane_electromagnetic_waves}

Consider a \emph{plane wave}\index{plane wave} in which $\mathbf{E}$ and $\mathbf{B}$ depend only on $(x, t)$ and not on $(y, z)$. A simple example is
\[
...
\nabla \times \mathbf{B} = \mu_0 \epsilon_0 \frac{\partial \mathbf{E}}{\partial t}.
\]
Of particular important is a \emph{monochromatic wave}\index{monochromatic wave} of a single \emph{angular frequency}\index{angular frequency} $\omega$, such as
\[
E = E_0 \cos(kx - \omega t), \qquad B = \frac{E_0}{c} \cos(kx - \omega t),
\]
where $E_0$ is a constant amplitude and $k = \frac{\omega}{c}$ is the \emph{wavenumber}\index{wavenumber}, related to the wavelength $\lambda$ by $k = \frac{2\pi}{\lambda}$. (The frequency is $\nu = \frac{\omega}{2 \pi}$ and the period is $\frac{1}{\nu} = \frac{2\pi}{\omega}$).

\begin{remark}
	\begin{itemize}
		\item[]
		\item The (angular) frequency and wavenumber are related by the \emph{dispersion relation}\index{dispersion relation}
			\[
			\omega^2 = c^2 k^2,
			\]
			i.e. $\omega = \pm ck$.
		\item The oscillations and $\mathbf{E}$ and $\mathbf{B}$ are in phase but in orthogonal directions.
		\item The waves are \emph{transverse}\index{transverse}: the oscillating fields are orthogonal to the direction in which the wave varies (and propogates).
	\end{itemize}
...
\begin{figure}[h]
	...
	\begin{tikzpicture}
		\begin{axis}[
			...
			\addplot3[domain = 0:2*pi, samples y = 1, blue] (x, {sin(deg(x))}, 0);
			\addplot3[domain = 0:2*pi, samples y = 1, red] (x, 0, {sin(deg(x))});
			\addplot3[domain = 0:2*pi] (x,0,0);
		\end{axis}
	\end{tikzpicture}
\end{figure}
...
\end{align*}
where $\mathbf{E}_0, \mathbf{B}_0$ are constant (complex) vector amplitudes, $\mathbf{k}$ is the (real and constant) \emph{wavevector}\index{wavevector} and $\omega$ is the (real and constant) angular frequency. The wavenumber is $k = |\mathbf{k}|$.
...
This is similar to the one-dimensional wave considered previously and corresponds to a \emph{linearly polarized wave}\index{linearly polarized wave}.
