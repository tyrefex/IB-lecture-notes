...
\begin{definition}
	...
	\begin{enumerate}[(i)]
		...
		\item $\phi$ is \textit{negative definite}\index{negative definite} if for all $u \ni V \setminus \{0\}$, $\phi(u, u) < 0$,
...
\begin{definition}
	...
	\item $\alpha$ is an \textit{isometry}\index{isometry} if
		...
		.\]
		Over $\mathbb{R}$, we say $\alpha$ is orthogonal, and over $\mathbb{C}$ we say $\alpha$ is unitary\index{unitary}.
	\end{enumerate}
\end{definition}
